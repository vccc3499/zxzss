...
(async()=>{
 ...
 await p.goto('https://www.avito.ru/profile/messenger/channel/u2i-8sT2scgnaqhOmtKj~jcD4g',{waitUntil:'domcontentloaded', timeout:90000}); await p.waitForTimeout(2000);
 const m=p.getByLabel('Открыть меню');
 const n=await m.count();
 console.log('menu buttons',n);
 if(n){await m.nth(n-1).click({timeout:8000}); await p.waitForTimeout(1000);} 
 const options = await p.evaluate(()=>Array.from(document.querySelectorAll('button,[role="menuitem"],li,a,div')).map(el=>(el.innerText||el.getAttribute('aria-label')||'').trim()).filter(Boolean).filter(t=>/удал|архив|заблок|пожал|спам|диалог|перепис|чат/i.test(t)).slice(0,100));
